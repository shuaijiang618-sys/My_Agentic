...
class SkillManager:
    ...
    def prompt_for(
        ...
        只注入匹配的 Skill；同 experiment 下按 user_id 稳定 A/B 分桶选一个 variant。
        """
        matched = [
            skill for skill in self._skills
            if skill.matches(message, agent_type)
        ]
        selected = self._select_ab_variants(matched, user_id or "anonymous")

        blocks: List[str] = []
        matched_detail: List[tuple[Skill, List[str]]] = []
        ...
        for skill in selected:
            ...
            blocks.append(block)
            matched_detail.append((skill, matched_keywords))
            remaining -= len(block)
            if remaining <= 0:
                break
        ...
        detail = "; ".join(
            ...
            f"{f'/{skill.variant}' if skill.variant else ''}"
            f"(keywords={', '.join(keywords) if keywords else 'all'})"
            for skill, keywords in matched_detail
        )
        logger.info(
            "Skills 已注入: agent=%s user=%s matched=%s message=%r",
            agent_type or "all",
            (user_id or "anonymous")[:8],
            detail,
            (message or "")[:80],
        )

        return (
            "以下是当前请求可用的 AI-Powered Medical Diagnostics Skills。"
            "请优先遵循这些业务规则；如果与系统角色冲突，以系统角色和安全边界为准。\n\n"
            + "\n\n".join(blocks)
        )

    @staticmethod
    def _ab_bucket(user_id: str, experiment: str) -> int:
        digest = hashlib.md5(f"{user_id}:{experiment}".encode()).hexdigest()
        return int(digest[:8], 16) % 100

    def _select_ab_variants(
        self, skills: List[Skill], user_id: str,
    ) -> List[Skill]:
        if not skills:
            return []

        by_experiment: Dict[str, List[Skill]] = defaultdict(list)
        standalone: List[Skill] = []
        for skill in skills:
            if skill.experiment:
                by_experiment[skill.experiment].append(skill)
            else:
                standalone.append(skill)

        selected: List[Skill] = list(standalone)
        for experiment, variants in by_experiment.items():
            enabled = [v for v in variants if v.enabled]
            if not enabled:
                continue
            if len(enabled) == 1:
                selected.append(enabled[0])
                continue
            ordered = sorted(enabled, key=lambda s: (s.variant, s.name))
            total_weight = sum(max(1, v.weight) for v in ordered)
            bucket = self._ab_bucket(user_id, experiment) / 100.0 * total_weight
            acc = 0.0
            picked = ordered[-1]
            for variant in ordered:
                acc += max(1, variant.weight)
                if bucket < acc:
                    picked = variant
                    break
    ...
    def _log_loaded_skills(self) -> None:
        ...
        lines = [
            "",
            "======== AI-Powered Medical Diagnostics Skills Loaded ========",
            f"目录: {self.root_dir}",
    ...
    def _load_json(self, path: Path) -> Optional[Skill]:
        ...
        return Skill(
            ...
            weight=self._as_int(raw.get("weight"), default=100),
        )
    ...
    def _load_text(self, path: Path) -> Optional[Skill]:
        ...
        return Skill(
            ...
            experiment=str(meta.get("experiment") or "").strip(),
            variant=str(meta.get("variant") or "").strip(),
            weight=self._as_int(meta.get("weight"), default=100),
        )
