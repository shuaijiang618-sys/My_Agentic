...
from monitor import metrics as prom
...
class PerformanceMonitor:
    ...
    def __init__(
        ...
        self._prom_enabled = prometheus_port is not None
    ...
    async def _collect(self) -> None:
        ...
        for agent_key, s in agent_stats.items():
            ...
            if self._prom_enabled:
                prom.AGENT_SUCCESS_RATE.labels(agent=agent_key).set(sr)
                prom.AGENT_LATENCY_MS.labels(agent=agent_key).observe(ms)
        ...
        for tool_name, s in tool_stats.items():
            ...
            if self._prom_enabled:
                prom.TOOL_SUCCESS_RATE.labels(tool=tool_name).set(sr)
